...
class TrajEncoder(nn.Module):
    ...
    def __init__(
        ...
        super(TrajEncoder, self).__init__() # Module class attribute initi.
...
class StateEncoder(nn.Module):
    """ """
    def __init__(
        self, state_type, h_dim=64, embedding_dim=64
        , mlp_dim=1024, num_layers=1,
        dropout=0.0
    ):
        ...
        self.spatial_embedding = nn.Linear( 
            state_size, embedding_dim # input sample size, output sample size
        ) 
    def init_hidden(self, batch):
...
class TrafficEncoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        # Encode Traffic Light 
        batch = obs_traj.size(1) # npeds
        # total = batch * (MAX_PEDS if padded else 1)
        obs_traj_embedding = self.spatial_embedding(obs_traj.contiguous().view(-1,1))  # (-1, input_size)
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        if pooling_type:
            ...
            input_dim = 3 * encoder_h_dim + bottleneck_dim + 512 # 64 + 1024 = 1088 ->64*3+1024=1216
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, 
        ...
        batch_size = img.size(1) # batch_size  // Issue : (final batch : 1, 2, 3 < batch_size) 
        img_h_dim= batch_size*512
        img_ft = img.contiguous().view(-1, img_h_dim) # tensor size to (batch, * ) 
        ...
        if self.pooling_type:
            ...
            mlp_decoder_context_input = torch.cat(
                [final_encoder_h.contiguous().view(-1, self.encoder_h_dim), # batch, 64, location [1, batch, 64] => [batch, 64]
                 ...
                 final_encoder_h3.contiguous().view(-1, self.encoder_h_dim), # 64, traffic
